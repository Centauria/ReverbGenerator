...
if __name__ == '__main__':
    ...
    for split in splits:
        ...
        phis = np.hstack(phis)
        print(f'{split} mics: {mic_xyz.shape}')
        print(f'{split} srcs: {source_xyz.shape}')
        ...
        g = sb.FacetGrid(
            data=pf, hue='label',
            palette=dict(src='blue', mic='seagreen'),
            height=5, xlim=[0, 5], ylim=[0, 10]
        )
        g.map(sb.scatterplot, 0, 1, s=1, alpha=0.3)
        g.add_legend()
        g.fig.subplots_adjust(top=0.95)
        g.ax.set_title(f'mic/source locations ({split})')
        plt.xlabel('dim_0')
        plt.ylabel('dim_1')
        plt.show()
        sb.histplot(rs)
        plt.title(f'r distribution ({split})')
        plt.xlabel(r'$r$')
        plt.show()
        ...
        plt.title(rf'$\theta$ distribution ({split})')
        plt.xlabel(r'$\theta$')
        plt.show()
        ...
        plt.title(rf'$\phi$ distribution ({split})')
        plt.xlabel(r'$\phi$')
        plt.show()
